...
namespace MPTickBar
{
    public class PluginUI : IDisposable
    {
        ...
        private struct MPTickBarUI
        {
            ...
            public TextureWrap Stack { get; set; }
        }
        ...
        private void RenderGaugeUIElement(MPTickBarUI mpTickBarUI, float uiScale, float offsetY, float elementWidth, float progress, int uiNumber, Vector4 tintColor)
        {
            var startX = 0.0f;
            var startY = offsetY;
        ...
        private void RenderJobStackUIElement(MPTickBarUI mpTickBarUI, float uiScale, float startX, float startY, int uiNumber, Vector4 tintColor)
        {
            var textureX = uiNumber * 0.5f;
            var scaledElementWidth = 20.0f * uiScale;
            var scaledElementHeight = 20.0f * uiScale;
            ImGui.SetCursorPos(new Vector2(startX, startY));
            ImGui.Image(mpTickBarUI.Stack.ImGuiHandle, new Vector2(scaledElementWidth, scaledElementHeight), new Vector2(textureX, 0.0f), new Vector2(textureX + 0.5f, 0.5f), tintColor);
        }
        ...
        private void DrawMPTickBar(bool isPreview)
        {
            ...
            this.RenderGaugeUIElement(mpTickBarUI, uiScale, offsetY, elementWidth, 1.0f, 5, Vector4.One);
            this.RenderGaugeUIElement(mpTickBarUI, uiScale, offsetY, elementWidth, progress, 2, new Vector4(this.Configuration.ProgressBarTintColor, 1.0f));
            this.RenderGaugeUIElement(mpTickBarUI, uiScale, offsetY, elementWidth, 1.0f, 0, Vector4.One);
            ...
            if (this.Configuration.IsFastFireIIIMarkerVisible && (this.IsUmbralIceIIIActivated || isPreview))
            {
                ...
                if (this.Configuration.FastFireIIIMarkerType == FastFireIIIMarkerType.Icon)
                {
                    ...
                    this.RenderJobStackUIElement(mpTickBarUI, uiScale, startX, startY, 0, Vector4.One);
                    this.RenderJobStackUIElement(mpTickBarUI, uiScale, startX, startY, 1, new Vector4(this.Configuration.FastFireIIIMarkerTintColor, 1.0f));
                }
